...
const Charts = () => {
  ...
  useEffect(() => {
    ...
    setStatusData([{ name: "Status", active: activeCount, deactive: deactiveCount }]);
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
      const dataName = payload[0].name || label || payload[0].payload?.name || "";
  ...
  return (
    <div style={{ color: "#fff" }}>
      <h1 style={{ marginBottom: "24px", fontSize: isMobile ? "24px" : "28px" }}>📊 Analytics Charts</h1>
      ...
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(auto-fit, minmax(300px, 1fr))", gap: "24px" }}>
        ...
        <div style={{ background: "#1c1c1c", borderRadius: "20px", padding: isMobile ? "16px" : "20px", border: "1px solid #2a2a2a" }}>
          <h3 style={{ textAlign: "center", marginBottom: "16px", fontSize: isMobile ? "16px" : "18px" }}>Gender Distribution</h3>
        ...
        <div style={{ background: "#1c1c1c", borderRadius: "20px", padding: isMobile ? "16px" : "20px", border: "1px solid #2a2a2a" }}>
          <h3 style={{ textAlign: "center", marginBottom: "16px", fontSize: isMobile ? "16px" : "18px" }}>User Status</h3>
          <ResponsiveContainer width="100%" height={250}>
            <BarChart data={statusData}>
              ...
              <Legend wrapperStyle={legendStyle} iconSize={12} />
              <Bar dataKey="active" fill="#4caf50" name="Active" barSize={isMobile ? 30 : 50} activeBar={{ fill: "#2e7d32" }} />
              <Bar dataKey="deactive" fill="#f44336" name="Deactive" barSize={isMobile ? 30 : 50} activeBar={{ fill: "#c62828" }} />
            </BarChart>
          </ResponsiveContainer>
        </div>
        ...
        {/* نمودار خطی رشد */}
        <div style={{ background: "#1c1c1c", borderRadius: "20px", padding: isMobile ? "16px" : "20px", border: "1px solid #2a2a2a" }}>
          <h3 style={{ textAlign: "center", marginBottom: "16px", fontSize: isMobile ? "16px" : "18px" }}>User Growth (Last 10)</h3>
          <ResponsiveContainer width="100%" height={250}>
            <LineChart data={trendData}>
              ...
              <Tooltip content={<CustomTooltip />} />
              <Legend wrapperStyle={legendStyle} iconSize={12} />
              <Line type="monotone" dataKey="users" stroke="#3b82f6" name="Total Users" activeDot={{ r: 6, fill: "#3b82f6" }} />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </div>
